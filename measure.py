...
assert version in ('8.3', '8.5') and first in ('tar', 'minimal')
...
with tempfile.TemporaryDirectory(prefix='php-darwin-extract-') as folder:
    ...
    for mode in [first, 'minimal' if first == 'tar' else 'tar']:
        ...
        if mode == 'tar':
            ...
            subprocess.run(['tar', '--ignore-zeros', '-xkmpf', str(archive), '--no-same-owner',
                            '--numeric-owner', '--no-mac-metadata', '--no-acls', '--no-xattrs',
                            '--no-fflags', '-C', str(destination)], check=True,
    ...
    with subprocess.Popen(['zstd', '-qdc', str(archive)], stdout=subprocess.PIPE) as decoder:
        with tarfile.open(fileobj=decoder.stdout, mode='r|', ignore_zeros=True) as contents:
            for member in contents:
                ...
                paths = [root / mode / member.name for mode in ('tar', 'minimal')]
                if member.isfile():
                    digest = hashlib.sha256()
    ...
    # An untimed filesystem trace identifies kernel work without distorting the
    # comparison above. It runs only on this disposable hosted runner.
    profile = root / 'profile'
    profile.mkdir()
    with open('filesystem.log', 'w') as log:
        tracer = subprocess.Popen(['sudo', '-n', '/usr/bin/fs_usage', '-w', '-f', 'filesystem', '-t', '15', 'tar'], stdout=log, stderr=subprocess.STDOUT)
        time.sleep(0.5)
        subprocess.run(['tar', '--ignore-zeros', '-xkmpf', str(archive), '--no-same-owner', '--numeric-owner', '-C', str(profile)], check=True)
        result['trace_status'] = tracer.wait(timeout=20)
    pathlib.Path('results.json').write_text(json.dumps(result, indent=2) + '\n')
