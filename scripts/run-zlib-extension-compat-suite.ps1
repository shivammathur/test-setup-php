...
$ExtensionMetadata = @(
  [PSCustomObject]@{ Package = 'memcache';   Artifact = 'memcache';   Module = 'memcache';   Ini = 'extension';      Required = $true;  Dependencies = @() },
  [PSCustomObject]@{ Package = 'memcached';  Artifact = 'memcached';  Module = 'memcached';  Ini = 'extension';      Required = $true;  Dependencies = @() },
  [PSCustomObject]@{ Package = 'oauth';      Artifact = 'oauth';      Module = 'OAuth';      Ini = 'extension';      Required = $true;  Dependencies = @() },
  [PSCustomObject]@{ Package = 'xdebug';     Artifact = 'xdebug';     Module = 'xdebug';     Ini = 'zend_extension'; Required = $true;  Dependencies = @() },
  [PSCustomObject]@{ Package = 'pecl_http';  Artifact = 'http';       Module = 'http';       Ini = 'extension';      Required = $true;  Dependencies = @('raphf') },
  [PSCustomObject]@{ Package = 'solr';       Artifact = 'solr';       Module = 'solr';       Ini = 'extension';      Required = $true;  Dependencies = @() },
  [PSCustomObject]@{ Package = 'ssh2';       Artifact = 'ssh2';       Module = 'ssh2';       Ini = 'extension';      Required = $true;  Dependencies = @() },
  [PSCustomObject]@{ Package = 'xlswriter';  Artifact = 'xlswriter';  Module = 'xlswriter';  Ini = 'extension';      Required = $true;  Dependencies = @() },
  [PSCustomObject]@{ Package = 'zip';        Artifact = 'zip';        Module = 'zip';        Ini = 'extension';      Required = $true;  Dependencies = @() }
)

$DependencyMetadata = @{
  raphf = [PSCustomObject]@{ Package = 'raphf'; Artifact = 'raphf'; Module = 'raphf'; Ini = 'extension'; Required = $false; Dependencies = @() }
}
...
function Get-WindowsPeclPackageUrl {
  ...
  $baseUrl = "https://windows.php.net/downloads/pecl/releases/$Package"
  try {
  ...
  foreach ($version in $versions) {
    ...
    $pattern = "php_$([regex]::Escape($Artifact))-$([regex]::Escape($version))-$([regex]::Escape($PhpSeries))-$tsPart-$([regex]::Escape($Vs))-$([regex]::Escape($Arch))\.zip"
...
foreach ($module in @('raphf')) {
  if (Test-Path -LiteralPath (Join-Path $extDir "php_$module.dll")) {
...
foreach ($meta in $ExtensionMetadata) {
  ...
  $dll = Join-Path $extDir "php_$($meta.Module.ToLowerInvariant()).dll"
  if ($meta.Module -eq 'OAuth') {
    $dll = Join-Path $extDir 'php_oauth.dll'
  }
