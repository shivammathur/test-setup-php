...
for suite in phpstan pgo-script pgo-script-opcache zend-bench untrained-extensions; do
  case "$suite" in
    phpstan) runs=3 ;;
    pgo-script|pgo-script-opcache|untrained-extensions) runs=5 ;;
    zend-bench) runs=10 ;;
  esac

  batch=0
  for variant in baseline optimized optimized baseline optimized baseline baseline optimized; do
    batch=$((batch + 1))
