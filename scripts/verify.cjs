...
for (const [version, tag, expected] of [
  ['2.40.0-alpha.1', '2.40.0-alpha.1', 'prerelease'],
  ['2.40.0-beta', '2.40.0-beta', 'prerelease'],
  ['2.40.0-RC1', 'v2.40.0-RC1', 'prerelease'],
  ['2.40.0', '2.40.0', 'latest']
]) {
  const payload = process.env.TEST_EVENT === 'release' ? {release:{tag_name:tag}} : {inputs:{tag}};
  ...
  results.push({event:process.env.TEST_EVENT,releaseTag:tag,name:result.name,version,channel:expected,files:result.entryCount,dryRun:true});
  console.log(`${process.env.TEST_EVENT}: ${tag} -> ${expected}, ${result.name}, ${result.entryCount} files`);
}
