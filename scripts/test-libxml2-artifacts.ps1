...
$acceptedPhpArchives = Get-Content "$PSScriptRoot/../tests/accepted-php-archives.json" -Raw | ConvertFrom-Json -AsHashtable
...
foreach ($zip in $zips) {
    ...
    try {
        if (-not $acceptedPhpArchives.ContainsKey($zip.Name) -or
            (Get-FileHash $zip.FullName -Algorithm SHA256).Hash -ne $acceptedPhpArchives[$zip.Name]) {
            throw 'PHP runtime archive differs from the accepted published-input build'
        }
