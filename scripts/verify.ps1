...
Import-Module (Join-Path $phpRoot 'PhpManager/PhpManager.psm1') -ErrorAction Stop
$startup = Get-PhpIniKey -Key display_startup_errors -Path "$phpRoot/php.ini"
if ($startup -ne 'On') { throw "Startup-error display changed: $startup" }
...
[ordered]@{
  phase=$Phase; runtime=$runtime; startup=$startup; metadata=$metadata; ini=$ini
