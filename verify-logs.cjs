  ...
  for (const job of jobs) {
    ...
    try {
      ...
      for (let i = 0; i < commands.length; i++) {
        ...
        const actionEnd = lines.findIndex((line, index) => index > commands[i].index && line.startsWith('##[group]Run node verify.cjs after ' + phase));
        assert(actionEnd > commands[i].index, phase + ': cannot find the end of the setup step');
        const segment = lines.slice(commands[i].index + 1, actionEnd);
        ...
        assert(!/example-(composer|packagist|json)-token/.test(segment.join('\n')), phase + ': credential appeared in setup output');
        entry.cases.push({phase, enabled, expectedTrace, executed: commands[i].path, traceLines: traceLines.length, assignments: assignments.length, credentialsProtected: true});
      }
    } catch (error) {
