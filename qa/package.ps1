...
$expectedFiles = @(
    'bin/libopenblas.dll', 'lib/libopenblas.lib', 'lib/libopenblas.exp',
    'include/cblas.h', 'include/f77blas.h', 'include/lapack.h', 'include/lapacke.h',
    'include/lapacke_config.h', 'include/lapacke_mangling.h', 'include/lapacke_utils.h',
    'include/openblas_config.h', 'share/sbom/OpenBLAS.cdx.json', 'share/sbom/OpenBLAS.spdx.json'
) + @($licenses | ForEach-Object { "share/licenses/$_" })
$root = (Resolve-Path install).Path
$actualFiles = @(Get-ChildItem install -File -Recurse | ForEach-Object {
    $_.FullName.Substring($root.Length + 1).Replace('\', '/')
})
$difference = Compare-Object $expectedFiles $actualFiles
if ($difference) { throw "Unexpected extension package layout: $($difference | Out-String)" }
...
'PASS: original extension file set, eight license notices, complete runtime SBOM components and PECL URLs.' | Tee-Object validation/package.txt | Write-Host
