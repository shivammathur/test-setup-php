...
const composerHome = process.platform === 'win32'
  ? path.join(process.env.APPDATA, 'Composer')
  : path.join(os.homedir(), '.composer');
const auth = JSON.parse(fs.readFileSync(path.join(composerHome, 'auth.json'), 'utf8'));
assert.equal(auth['github-oauth']['github.com'], 'example-composer-token');
assert.equal(auth['http-basic']['repo.packagist.com'].password, 'example-packagist-token');
assert.equal(auth.bearer['example.org'], 'example-json-token');
const record = {
