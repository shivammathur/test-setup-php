...
keg="$brew_prefix/Cellar/$PHP_FORMULA/5.6.40_$PHP_REVISION"
opt="$brew_prefix/opt/$PHP_FORMULA"
config_dir="$brew_prefix/etc/php/5.6$PHP_SUFFIX"
pear_dir="$brew_prefix/share/pear@5.6$PHP_SUFFIX"

# Check the actual published tap and packaged recipe before rerunning any hook.
git -C "$tap_repo" log -1 --oneline
git -C "$tap_repo" merge-base --is-ancestor "$EXPECTED_TAP_COMMIT" HEAD
test -d "$keg"
...
php --ini
php -m
php -r 'if (PHP_VERSION !== "5.6.40" || (bool) PHP_DEBUG !== (getenv("PHP_EXPECT_DEBUG") === "true") || (bool) PHP_ZTS !== (getenv("PHP_EXPECT_ZTS") === "zts")) { fwrite(STDERR, "Wrong PHP version or build variant\n"); exit(1); }'
...
test -f "$opt/lib/php/$abi/opcache.so"
grep -F "$opt/lib/php/$abi/opcache.so" "$config_dir/conf.d/ext-opcache.ini"
...
for channel in doc.php.net pear.php.net pecl.php.net; do
  grep -F "Update of Channel \"$channel\" succeeded" validation-logs/postinstall.log
done
...
check_pear php_ini "$config_dir/php.ini"
check_pear ext_dir "$brew_prefix/lib/php/pecl/$abi"
printf 'PASS: %s 5.6.40_%s installed from a rebuilt bottle\n' "$PHP_FORMULA" "$PHP_REVISION"
