...
function assert_intl_userland_manifest(): void
{
    ...
    if (PHP_VERSION_ID >= 80600) {
        $localeFunctions[] = 'locale_get_display_keyword';
        $localeFunctions[] = 'locale_get_display_keyword_value';
        $localeMethods[] = 'getDisplayKeyword';
        $localeMethods[] = 'getDisplayKeywordValue';
    }
    assert_internal_function_manifest('intl locale', 'locale_', $localeFunctions, [
    ...
    $datePatternGeneratorMethods = ['__construct', 'create', 'getBestPattern'];
    if (PHP_VERSION_ID >= 80600) {
        $datePatternGeneratorMethods[] = 'getBaseSkeleton';
        $datePatternGeneratorMethods[] = 'getSkeleton';
    }
    assert_class_method_manifest(IntlDatePatternGenerator::class, $datePatternGeneratorMethods);
    ...
    assert_class_method_manifest(Locale::class, $localeMethods, ['addLikelySubtags', 'isRightToLeft', 'minimizeSubtags']);
    ...
    $spoofcheckerMethods = ['__construct', 'areConfusable', 'isSuspicious', 'setAllowedLocales', 'setChecks', 'setRestrictionLevel'];
    if (PHP_VERSION_ID >= 80600) {
        $spoofcheckerMethods[] = 'getSkeleton';
        if (version_compare(INTL_ICU_VERSION, '74.0', '>=')) {
            $spoofcheckerMethods[] = 'areBidiConfusable';
            $spoofcheckerMethods[] = 'getBidiSkeleton';
        }
    }
    assert_class_method_manifest(Spoofchecker::class, $spoofcheckerMethods, ['setAllowedChars']);
...
function test_intl_locale_grapheme_normalizer_char_idn(): void
{
    ...
    try {
        ...
        if (method_exists(Locale::class, 'getDisplayKeyword')) {
            $displayKeyword = Locale::getDisplayKeyword('calendar', 'en');
            assert_true(
                is_string($displayKeyword) && str_contains($displayKeyword, 'Calendar'),
                'Locale display keyword should localize the calendar keyword'
            );
            assert_same(
                $displayKeyword,
                locale_get_display_keyword('calendar', 'en'),
                'Procedural and object-oriented display keyword APIs should agree'
            );

            $displayKeywordValue = Locale::getDisplayKeywordValue('de_DE@calendar=gregorian', 'calendar', 'en');
            assert_true(
                is_string($displayKeywordValue) && str_contains($displayKeywordValue, 'Gregorian'),
                'Locale display keyword value should localize the Gregorian calendar'
            );
            assert_same(
                $displayKeywordValue,
                locale_get_display_keyword_value('de_DE@calendar=gregorian', 'calendar', 'en'),
                'Procedural and object-oriented display keyword value APIs should agree'
            );
        }
...
function test_intl_collator_number_message_date(): void
{
    ...
    if (method_exists(IntlDatePatternGenerator::class, 'getSkeleton')) {
        assert_same('MMMdd', IntlDatePatternGenerator::getSkeleton('dd/MMM'), 'IntlDatePatternGenerator should extract a skeleton');
        assert_same('MMMd', IntlDatePatternGenerator::getBaseSkeleton('dd/MMM'), 'IntlDatePatternGenerator should extract a base skeleton');
    }
}
...
function test_intl_iterators_transliterator_spoof_resource_converter(): void
{
    ...
    if (method_exists($spoof, 'getSkeleton')) {
        assert_same(
            $spoof->getSkeleton('c'),
            $spoof->getSkeleton("с"),
            'Spoofchecker skeletons should match for confusable Latin and Cyrillic characters'
        );
    }
    if (method_exists($spoof, 'areBidiConfusable')) {
        $leftToRightA = "A1א";
        $leftToRightB = "Aא1";
        assert_true(
            $spoof->areBidiConfusable(Spoofchecker::LTR, $leftToRightA, $leftToRightB),
            'Spoofchecker should detect left-to-right bidi confusables'
        );
        assert_false(
            $spoof->areBidiConfusable(Spoofchecker::RTL, $leftToRightA, $leftToRightB),
            'Spoofchecker should distinguish the same strings in right-to-left context'
        );
        assert_same(
            $spoof->getBidiSkeleton(Spoofchecker::LTR, $leftToRightA),
            $spoof->getBidiSkeleton(Spoofchecker::LTR, $leftToRightB),
            'Spoofchecker bidi skeletons should match for left-to-right confusables'
        );
    }
