...
published PECL binaries are reported separately. The `memcached` smoke keeps the
compressed set/get round trip on 64-bit PHP; on 32-bit PHP it validates the
compression option surface because the stock-zlib baseline shows the same
memcached client failures before zlib-rs is involved.
